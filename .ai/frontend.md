...
## Tujuan
Dokumen ini menjelaskan struktur arsitektur, modul pemrosesan, dan standardisasi pengembangan frontend (Vite, CSS Tailwind v4, jQuery/Datatables, AlpineJS, Preline UI) pada proyek berbasis arsitektur ini.
...
3. **Reaktivitas Form Dinamis**: Untuk modul form yang memiliki tabel dinamis, hitung otomatis, dan input bertingkat, proyek menggunakan **AlpineJS** yang diintegrasikan langsung dengan pembungkus PageScript.
...
## Struktur
### Folder Aset JS & CSS
- `resources/js/pages/{module}/`: Berisi berkas JS spesifik halaman (misal: `index.js`, `form.js`, `alpine-component.js`).
- `resources/js/utils/`: Berisi berkas utilitas global, antara lain:
  ...
  - `ui-init.js` / `reinit-ui.js`: Utilitas untuk memicu kembali deteksi elemen Preline UI setelah perubahan DOM secara dinamis.
  - `normalize-form.js`: Menghilangkan format rupiah/angka dari form payload sebelum dikirim ke backend.
  - `week.js`: Pustaka manipulasi tanggal dan filter nomor minggu.

## Contoh implementasi
Inisialisasi dasar script halaman menggunakan module pattern:
- Berkas JS: `resources/js/pages/{module}/index.js`
- Pemicu AJAX: `resources/js/utils/api-provider.js`
...
const PageScript = (function () {
    const bindEvents = () => {
        $(document).on("click", ".btn-action", async function () {
            ...
            // Eksekusi AJAX
            const res = await ApiProvider.get(route("categories.show", id));
