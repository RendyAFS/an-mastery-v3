<div id="memo-form-fields" class="space-y-5">
    ...
    <div class="grid grid-cols-2 gap-3">
        <div class="space-y-1">
            ...
            <div class="relative">
                <span
                    class="absolute inset-y-0 start-0 flex items-center pl-3 text-sm text-(--color-dark-gray)">Rp</span>
                <input type="text" inputmode="numeric" id="nominal" name="nominal" placeholder="0"
                    class="pl-9 pr-3 py-2 block w-full text-sm rounded-lg bg-(--color-light) border border-(--color-gray)
                        text-(--color-dark) focus:border-(--color-primary) focus:ring focus:ring-(--color-primary)/30
                        dark:bg-(--color-dark) dark:border-(--color-slate) dark:text-(--color-light)">
            </div>
        </div>
