@php
    $detailsJson = ($sablon->sablonDetails ?? collect())->map(fn($d) => [
        'fabric_detail_id' => $d->fabric_detail_id,
        'color_fabric_id'  => $d->color_fabric_id,
        'long_fabric'      => $d->long_fabric,
    ])->values();

    $employeeDetailsJson = ($sablon->sablonEmployeeDetails ?? collect())->map(fn($d) => [
        'fabric_detail_id'   => $d->fabric_detail_id,
        'employee_id'        => $d->employee_id,
        'layers'             => $d->layers,
        'fee'                => $d->fee,
        'additional_fee'     => $d->additional_fee, // {nominal, notes}
        'total'              => $d->total,
...
<div x-data="sablonForm(
    ...
    {{-- ===== CARD 1: Info Utama ===== --}}
    <div class="bg-(--color-light) dark:bg-(--color-dark-slate)/40 border border-(--color-gray) dark:border-(--color-slate) rounded-xl p-5 mb-6">
        <h3 class="text-sm font-semibold uppercase tracking-wide text-(--color-dark-gray) mb-4">Info Utama</h3>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            {{-- <div class="mb-2 space-y-2"> --}}
                <x-select id="supplier_id" name="supplier_id" label="Supplier" :options="$suppliers"
                    :value="$sablon?->supplier_id ?? null" placeholder="Choose Supplier"
                    search-placeholder="Search supplier..." clearable="true"
                    x-on:change="onSupplierChange($event)" />
            {{-- </div> --}}

            <div class="mb-2 space-y-2">
                <x-select id="fabric_id" name="fabric_id" label="Fabric" :options="$fabrics"
                    :value="$sablon?->fabric_id ?? null" placeholder="Choose Fabric"
                    search-placeholder="Search fabric..." clearable="true"
            ...
            <div class="mb-2 space-y-2">
                <x-select id="image_fabric_id" name="image_fabric_id" label="Image Fabric" :options="$imageFabrics"
                    :value="$sablon?->image_fabric_id ?? null" placeholder="Choose Image Fabric"
                    search-placeholder="Search image fabric..." clearable="true" />
            </div>

            <div class="mb-2 space-y-2">
                <x-select id="type_color_id" name="type_color_id" label="Type Color" :options="$typeColors"
                    :value="$sablon?->type_color_id ?? null" placeholder="Choose Type Color"
                    search-placeholder="Search type color..." clearable="true"
                    x-on:change="onTypeColorChange($event)" />
            </div>

            <div class="mb-2 space-y-2">
                <x-select id="type_fabric_id" name="type_fabric_id" label="Type Fabric" :options="$typeFabrics"
                    :value="$sablon?->type_fabric_id ?? null" placeholder="Choose Type Fabric"
                    search-placeholder="Search type fabric..." clearable="true" />
            </div>

            <div class="mb-2 space-y-2">
                <x-select id="price_employee_id" name="price_employee_id" label="Price Employee" :options="$priceEmployees"
                    :value="$sablon?->price_employee_id ?? null" placeholder="Choose Price Employee"
                    search-placeholder="Search..." clearable="true"
            ...
            <div class="mb-2 space-y-2">
                <x-select id="status" name="status" label="Status" :options="[
                    ...
                    'DELIVERED' => 'Delivered',
                    'RETURNED' => 'Returned',
                ]" :value="$sablon?->status->value ?? 'ON_PROGRESS'" placeholder="Choose Status" />
            </div>

            <div class="md:col-span-3 mb-2 space-y-2">
                <label for="notes" class="block text-sm font-medium text-(--color-dark) dark:text-(--color-light)">Notes</label>
    ...
    <div class="bg-(--color-light) dark:bg-(--color-dark-slate)/40 border border-(--color-gray) dark:border-(--color-slate) rounded-xl p-5 mb-6">
        ...
        <div class="relative overflow-visible rounded-lg border border-(--color-gray) dark:border-(--color-slate)">
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-(--color-light-gray) dark:bg-(--color-dark-slate) text-(--color-dark) dark:text-(--color-light)">
                        <th class="px-4 py-3 text-left font-semibold" style="width: 5%">No</th>
                        <th class="px-4 py-3 text-left font-semibold" style="width: 45%">Fabric Detail (Color - Stock)</th>
                ...
                <tbody>
                    <template x-for="(row, index) in fabricRows" :key="row.uid">
                        <tr class="border-t border-(--color-gray) dark:border-(--color-slate)">
                                ...
                                <div class="relative" @click.outside="row.open = false">
                                    ...
                                    <div x-show="row.open" x-cloak
                                        ...
                                        <ul class="max-h-48 overflow-y-auto py-1">
                                            <template x-for="option in filteredFabricDetails(row.search)" :key="option.id">
                                            ...
                                            <template x-if="filteredFabricDetails(row.search).length === 0">
                                                <li class="px-4 py-2 text-sm text-(--color-dark-gray)">No fabric detail found for selected Fabric</li>
                                            </template>
                                        </ul>
                                    </div>
                                </div>
                            ...
                            <td class="px-4 py-3">
                                <input type="number" min="0" step="0.01" x-model="row.long_fabric" required
    ...
    <div class="bg-(--color-light) dark:bg-(--color-dark-slate)/40 border border-(--color-gray) dark:border-(--color-slate) rounded-xl p-5">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-sm font-semibold uppercase tracking-wide text-(--color-dark-gray)">
            ...
            <template x-for="(row, index) in employeeRows" :key="row.uid">
                <div class="rounded-lg border border-(--color-gray) dark:border-(--color-slate) p-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                        {{-- poin #4: style select sama seperti x-select (custom dropdown) --}}
                        <div class="space-y-1">
                        ...
                        {{-- poin #7: additional_fee json {nominal, notes} --}}
                        <div class="space-y-1">
                            <label class="text-xs font-medium text-(--color-dark-gray)">Additional Fee (Nominal)</label>
                            <input type="number" min="0" step="0.01" x-model.number="row.additional_fee.nominal"
                                class="px-3 py-2 w-full rounded-lg bg-(--color-light-gray) border border-(--color-gray)
                                   text-(--color-dark) dark:bg-(--color-dark-slate) dark:border-(--color-slate) dark:text-(--color-light)">
                        </div>

                        <div class="space-y-1 md:col-span-2">
                            <label class="text-xs font-medium text-(--color-dark-gray)">Additional Fee (Notes)</label>
                            <input type="text" x-model="row.additional_fee.notes"
                                class="px-3 py-2 w-full rounded-lg bg-(--color-light-gray) border border-(--color-gray)
                                   text-(--color-dark) dark:bg-(--color-dark-slate) dark:border-(--color-slate) dark:text-(--color-light)">
                        </div>
                        ...
                        <div class="flex items-center gap-2 pt-5">
                            <input type="checkbox" x-model="row.is_change" class="checkbox-custom" :id="'change_' + row.uid">
                            <label :for="'change_' + row.uid" class="text-sm text-(--color-dark) dark:text-(--color-light)">Ganti Pekerja?</label>
                        </div>
                        ...
                        <div class="flex items-center gap-2 pt-5">
                            <input type="checkbox" x-model="row.is_payed" class="checkbox-custom" :id="'payed_' + row.uid">
                            <label :for="'payed_' + row.uid" class="text-sm text-(--color-dark) dark:text-(--color-light)">Sudah Dibayar</label>
                        </div>
                    </div>
                </div>
            </template>
