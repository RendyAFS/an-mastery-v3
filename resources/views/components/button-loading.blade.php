@props([
    'type' => 'button',
    'text' => __('button-loading.Submit'),
    'loadingText' => __('button-loading.Loading...'),
...
<button type="{{ $type }}" data-button-loading data-loading-text="{{ $loadingText }}"
    {{ $attributes->merge([
        'class' => "inline-flex items-center justify-center gap-x-2 font-medium transition-all duration-200 disabled:opacity-50 disabled:pointer-events-none $size $rounded $color $textColor",
    ]) }}>
