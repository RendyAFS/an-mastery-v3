...
const PageScript = (function () {
    ...
    const initDataTable = () => {
        datatable = initDatatable({
            table: "#type-colors-datatable",
            filterSelector: "#filter-type-colors",
            onRowClick: (row) => handleEdit(row.id),
            ajax: {
                url: route("type_colors.index"),
                method: "GET",
                dataSrc: "data",
                data: function (d) {
                    d.filter = $("#filter-type-colors").val();
                },
            },
            columns: [
                {
    ...
    const openModal = () => {
        HSOverlay.open("#hs-type-color-modal");
    };

    const closeModal = () => {
        HSOverlay.close("#hs-type-color-modal");
    };

    const setModalTitle = (title) => {
        $("#hs-type-color-modal-label").text(title);
    };
    ...
    const submitForm = async (submitter) => {
        ...
        try {
            if (mode === "create") {
                await ApiProvider.post(route("type_colors.store"), payload);
                Toast.success("Success", "Type Color Successfully Created");
            }
    ...
    const handleEdit = async (id) => {
        setModalTitle("Edit Type Color");
        setFormMode("edit", id);

        try {
            const response = await ApiProvider.get(
                route("type_colors.show", id),
            );
            fillForm(response.data);
            openModal();
        } catch (error) {
            console.error("Fetch type color error:", error);
            closeModal();
        }
    };
    ...
    const handleDelete = async (id) => {
        const confirmed = await Confirm.delete(
            "Are you sure you want to delete this type color? This action cannot be undone.",
        );

        if (!confirmed) return;

        try {
            await ApiProvider.delete(route("type_colors.destroy", id));
            Toast.success("Success", "Type color deleted successfully");
            reloadDatatable();
        } catch (error) {
            console.error("Delete type color error:", error);
        }
    };

    const handleRestore = async (id) => {
        const confirmed = await Confirm.show(
            "Restore this type color?",
            "Confirmation",
        );
        ...
        try {
            await ApiProvider.put(route("type_colors.restore", id));
            Toast.success("Success", "Type color restored");
            reloadDatatable();
        } catch (error) {
            console.error("Restore type color error:", error);
        }
    };

    const handleForceDelete = async (id) => {
        const confirmed = await Confirm.delete(
            "This will permanently delete the type color. Continue?",
        );

        if (!confirmed) return;

        try {
            await ApiProvider.delete(route("type_colors.force-delete", id));
            Toast.success("Success", "Type color permanently deleted");
            reloadDatatable();
        } catch (error) {
            console.error("Force delete type color error:", error);
        }
    };

    const bindEvents = () => {
        $(document).on("click", "#btn-create-type-color", () => {
            handleCreate();
        });
        ...
        document
            .getElementById("hs-type-color-modal")
    ...
    return {
        init() {
            form = document.getElementById("type-color-form");
