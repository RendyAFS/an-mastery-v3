    ...
    async function submitForm(action, submitter) {
        ...
        try {
            if (mode === "create") {
                ...
                flashToast("success", "Success", "Color Fabric Successfully Created");
                window.location.href = route("color_fabrics.index");
            }

            if (mode === "edit") {
                await ApiProvider.put(route("color_fabrics.update", id), payload);
                flashToast("success", "Success", "Color Fabric Successfully Updated");
                window.location.href = route("color_fabrics.index");
            }
        } catch (error) {
