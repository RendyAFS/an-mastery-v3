...
const PageScript = (function () {
    ...
    const CardGrid = () => {
        cardgrid = initCardgrid({
            containerId: "#gallery-cardgrid",
            filterSelector: "#filter-gallery",
            ajax: {
                url: route("galleries.index"),
            },
            renderCard,
            pageLength: 12,
            cardClickRoute: (row) => route("galleries.edit", row.id),
        });
    };
    ...
    const bindEvents = () => {
        ...
        $(document).on("click", ".btn-delete", async function () {
            ...
            if (!confirmed) return;
            try {
                await ApiProvider.delete(route("galleries.destroy", id));
                Toast.success(
        ...
        $(document).on("click", ".btn-restore", async function () {
            ...
            if (!confirmed) return;
            await ApiProvider.put(route("galleries.restore", id));
            Toast.success(
                window.langCustomAlert.success,
        ...
        $(document).on("click", ".btn-force-delete", async function () {
            ...
            if (!confirmed) return;
            await ApiProvider.delete(route("galleries.force-delete", id));
            Toast.success(
                window.langCustomAlert.success,
